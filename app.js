...
function change_button_text() {
    // 变更坐标系
    let temp = ne.innerText;
    ne.innerText = nw.innerText;
    nw.innerText = sw.innerText;
    sw.innerText = se.innerText;
    se.innerText = temp;
}
function pipe_join() {
    // 相交打断
...
function add_stamp() {
    ...
    stamp_count += 1;
    document.getElementById("stamp_count").innerText = `当前标记个数:${String(stamp_count)}`;
...
function goto_stamp(index=stamp_index) {
    if (index > stamp_count){
        alert("超出已有标记个数，请重试！");
        return
    }
    stamp_id = index - 1;
    t.nowX = stamps[stamp_id][0];
    t.nowY = stamps[stamp_id][1];
    draw_cursor();
}
...
function font_setting(flag) {
    let form = document.getElementById("font_setting");
    document.getElementById('font_setting_page').close();
    if(flag===1) {
        color = form.elements[0].value;
        font = form.elements[1].value + "px " + form.elements[2].value;
    }
}
function add_word(){
    // 添加文字
...
function draw_coordinates() {
    ...
    fluent();
    setPos(8);
    ctx.fillText(ne.innerText, t.nowX, t.nowY);
    t.nowX = x;
    t.nowY = y;
    set_up();
    draw(32);
    ...
    fluent();
    setPos(8);
    ctx.fillText(nw.innerText, t.nowX - 16, t.nowY);
    t.nowX = x;
    t.nowY = y;
    set_se();
    draw(40);
    ctx.fillText(se.innerText, t.nowX, t.nowY + 14);
    t.nowX = x;
    t.nowY = y;
    set_sw();
    draw(40);
    ctx.fillText(sw.innerText, t.nowX - 16, t.nowY + 14);
    t.nowX = x;
    t.nowY = y;
    set_dn();
    draw(40);
